...
class StylesManager {
  ...
  /**
   * Geeft de stijlindex terug voor "dezelfde stijl als sourceStyleIdx, maar
   * met een ander numFmtId". Zelfde logica als de Node-versie in
   * xlsx-direct.js -- moet in sync blijven.
   */
...
async function brVulIngredientRijen(writer, bundel, ingredientMap, overloop, stylesManager) {
  ...
  for (const rol of rollen) {
    ...
    if (dynamischeBlokken[rol]) {
      ...
      // Brewing-toevoegingen zijn normaal x1 (per individueel brouwsel),
      // BEHALVE als de regel is gemarkeerd met alles_in_brouwsel_1 ("All in
      // brew 1?" in de app): dan hele batch in één keer bij het eerste
      // brouwsel, dus net als Cellar x aantal brouwsels van DEZE batch.
      // Zichtbaar met een '*' achter het getal in kolom G + uitleg in Q.
